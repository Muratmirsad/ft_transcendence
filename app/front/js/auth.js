...
export function setupLoginForm() {
  ...
  document
    ...
    .addEventListener("submit", function (event) {
      ...
      fetch("https://176.41.7.226/api/login/", {
        method: "POST",
        headers: {
          ...
          if (data.token) {
            if (parseJWT(data.token).twofactoractive) {
              ...
              const socket = new WebSocket(
                "wss://176.41.7.226/ws/status/userstatus/"
              );
              socket.onopen = () => {
                socket.send("0");
              };
  ...
  login42Link.addEventListener("click", function (event) {
    ...
    window.location.href =
      "https://api.intra.42.fr/oauth/authorize?client_id=u-s4t2ud-466f8ff9df025fc510352c0b2b970477cd8618f031b1e7e196e8f573b492ae53&redirect_uri=https%3A%2F%2F176.41.7.226&response_type=code";
  });
}
...
export function setupRegisterForm() {
  document
    ...
    .addEventListener("submit", function (event) {
      ...
      fetch("https://176.41.7.226/api/register/", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
...
export function logout() {
  ...
  fetch("https://176.41.7.226/api/logout/", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
    ...
    .then((response) => {
      ...
      const socket = new WebSocket("wss://176.41.7.226/ws/status/userstatus/");
      socket.onopen = () => {
        socket.send("0");
        socket.close();
      };
